...
from .LinearSubdivisionFilter import LinearSubdivisionFilter as lsf
import numpy as np
import math 
import os
import sys
...
def ScaleSurf(surf, mean_arr = None, scale_factor = None, copy=True):
    ...
    # print("Shape Points:", max(shape_points))
...
def GetTubeFilter_test(vtkpolydata):
...
def ExtractFiber_test(surf, list_random_id) :
    ids = vtk.vtkIdTypeArray()
    ids.SetNumberOfComponents(1)
    ids.InsertNextValue(list_random_id) 
...
def TensorsToPolyData(verts, faces, edges):
    surf = vtk.vtkPolyData()
    points = vtk.vtkPoints()
    cells = vtk.vtkCellArray()
    ...
    for f in faces:
        t_vtk = vtk.vtkIdList()
        t_vtk.InsertNextId(f[0])
        t_vtk.InsertNextId(f[1])
        t_vtk.InsertNextId(f[2])
        cells.InsertNextCell(t_vtk)
